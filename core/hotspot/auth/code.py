...
def generate_code(user_fp):
    ...
    cache.set(f'code:value:{user_fp}', code, 300)
    cache.set_raw(f'code:attempts:{user_fp}', 0, 300)
    cache.set(f'code:sended:{user_fp}', False, 60)
    return code

def get_code(user_fp):
    return cache.get(f"code:value:{user_fp}")

def set_sended(user_fp):
    cache.set(f'code:sended:{user_fp}', True, 60)

def increment_attempts(user_fp):
    return cache.incr(f"code:attempts:{user_fp}")

def verify_code(user_fp, code: str):
    cached = cache.get(f"code:value:{user_fp}")
    if cached:
        return cached == code
...
def code_sended(user_fp):
    sended = cache.get(f"code:sended:{user_fp}")
    return sended is not None and sended

def clear_code(user_fp):
    cache.delete(f'code:value:{user_fp}')
    cache.delete(f'code:attempts:{user_fp}')
    cache.delete(f'code:sended:{user_fp}')
