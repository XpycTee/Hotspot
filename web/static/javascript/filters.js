document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('.select-toggle').forEach(toggle => {
        const options = Array.from(toggle.querySelectorAll('.select-option'));
        const slider = toggle.querySelector('.select-slider');
        const n = options.length || 1;

        const toggleInput = document.getElementById(toggle.dataset.target);
        if (toggleInput) {
            toggleInput._origValue = toggleInput.value;
        }
